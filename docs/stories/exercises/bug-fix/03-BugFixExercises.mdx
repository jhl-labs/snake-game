...
<Meta title="Exercises/버그 수정 실습/버그 수정 실습" />
...
프로젝트에 `exercises/basics/buggy_snake.c` 파일이 준비되어 있습니다.
**의도적으로 삽입된 6개의 버그**가 있습니다.

![](018-bug-01.png)

이 파일을 복사해서 실습을 진행합시다. 주의할 사항은 주석은 모두 제거해야 합니다.

agent에게 정답지를 알려주면 안되니깐요
...
<Source
  language="text"
  code={`
exercises/basics/buggy_snake.c의 snake_grow 함수에서
뱀 길이가 MAX_SNAKE_LENGTH를 초과할 때
...
<Source
  language="text"
  code={`
> exercises/basics/buggy_snake.c 파일에 여러 버그가 있어.
...
<Mermaid chart={`
flowchart TB
    subgraph BugTypes["6가지 버그 유형"]
        Array["1️⃣ 배열 경계<br/>(초급)"]
        Wall["2️⃣ 경계 조건<br/>(초급)"]
        Memory["3️⃣ 메모리 누수<br/>(초급)"]
        Direction["4️⃣ 방향 로직<br/>(중급)"]
        Collision["5️⃣ 충돌 체크<br/>(중급)"]
        Logic["6️⃣ 방향 전환<br/>(중급)"]
    end

    Array --> Wall --> Memory
    Direction --> Collision --> Logic
`} />
...
| # | 버그 | 난이도 | 수정 완료 |
|---|------|--------|----------|
| 1 | 배열 경계 체크 누락 (snake_grow) | 초급 | ☐ |
| 2 | 벽 충돌 경계 조건 오류 (is_wall_collision) | 초급 | ☐ |
| 3 | 메모리 해제 누락 (get_direction_name) | 초급 | ☐ |
| 4 | 방향 이동 로직 오류 (snake_move - LEFT) | 중급 | ☐ |
| 5 | 자기 충돌 체크 시작 인덱스 오류 (is_self_collision) | 중급 | ☐ |
| 6 | 반대 방향 전환 체크 없음 (snake_set_direction) | 중급 | ☐ |
